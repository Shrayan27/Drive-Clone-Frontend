...
export const useCollaboration = (fileId?: string) => {
  ...
  useEffect(() => {
    ...
    socket.on('user-joined-file', (data: { user: CollaborationUser; totalUsers: number }) => {
    ...
    socket.on('user-left-file', (data: { userId: string; totalUsers: number }) => {
      setCollaborationState(prev => ({
        ...prev,
        users: prev.users.filter(u => u.id !== data.userId),
        cursors: prev.cursors.filter(c => c.userId !== data.userId),
        selections: prev.selections.filter(s => s.userId !== data.userId)
      }));
    ...
    socket.on('cursor-updated', (data: { userId: string; x: number; y: number }) => {
      setCollaborationState(prev => ({
        ...prev,
        cursors: prev.cursors.filter(c => c.userId !== data.userId).concat({
          userId: data.userId,
          x: data.x,
          y: data.y
        })
    ...
    socket.on('selection-updated', (data: { userId: string; start: number; end: number }) => {
      setCollaborationState(prev => ({
        ...prev,
        selections: prev.selections.filter(s => s.userId !== data.userId).concat({
          userId: data.userId,
          start: data.start,
          end: data.end
        })
    ...
    socket.on('text-changed', (data: { userId: string; changes: any[]; timestamp: number }) => {
    ...
    socket.on('user-stopped-typing', (data: { userId: string }) => {
      setTypingUsers(prev => {
        ...
        const user = collaborationState.users.find(u => u.id === data.userId);
        if (user) {
  ...
  // Send text changes
  const sendTextChange = useCallback((changes: any[]) => {
    if (!socketRef.current || !fileId) return;
